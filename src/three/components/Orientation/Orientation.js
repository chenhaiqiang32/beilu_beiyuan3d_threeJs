...
import { sceneChange } from './../dataProgress';
...
export class Orientation {
  ...
  get followId() {
    return this.followModule.id;
  }

  set followId(id) {
    if (!this.has(id)) return;
    this.followModule.id = id;
  }
  ...
  constructor(core) {
    ...
    /** 人员跟踪模块 */
    this.followModule = new Follow(this);
  ...
  #add(item,sceneChanged) {
    ...
    if (item.sceneType === Orientation.SCENE_TYPE.INDOOR) {
      ...
      if (!indoorObj[buildingId][item.originId]) indoorObj[buildingId][item.originId] = [];
  ...
  #process(item,sceneChanged) {
    ...
    item.position = getCurrentPosition({ coordinate,originId,sceneType,},true);
  ...
  /** 取消搜索 */
  clearSearch() { // 这个地方不能通知前端关闭弹窗,前端触发的前端会自己关
  ...
  /** 开始跟踪 */
  startFollow() {
    if (!this.searchId) { // 跨场景跟踪的时候执行以下搜索
      this.searchId = this.followId;
      this.search();
    }
    this.followModule.startFollow();
    // this.orientation3D.setDialogVisible(true);
  }

  /** 取消跟踪 */
  cancelFollow() {
    // this.orientation3D.setDialogVisible(false);
    this.followModule.cancelFollow();
  }
